...
func TestMessagesPrepareBasic(t *testing.T) {
	...
	if !strings.HasPrefix(got, "<|begin▁of▁sentence|><|User|>Hello") {
		t.Fatalf("expected direct user prompt at the start, got %q", got)
	}
...
func TestMessagesPrepareArrayTextVariants(t *testing.T) {
	...
	if strings.Contains(got, "Output integrity guard") {
		t.Fatalf("did not expect output integrity guard in %q", got)
	}
}
